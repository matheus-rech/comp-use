...
*   UPDATE: 26 July 2004, 31 August 2004, 15 June 2005
*
*   DOCUMENTATION:
...
*
*
*   Copyright (c) July 2004, August 2004  Michael Thomas Flanagan
*
*   PERMISSION TO COPY:
...
public class FourierTransform extends Canvas{
    ...
    private int dataLength = 0;             // number of data points, i.e. length of ComplexData array
    private int dataLengthOriginal = 0;     // original DataLength value; the working data length be altered by deletion or padding
	private boolean dataAltered = false;    // set to true if dataLength altered, e.g. by point deletion or padding.
    ...
    private int fftDataLength = 0;          // the smallest power of two that is either equal to dataLength or larger than dataLength
	...
	public FourierTransform(double[] realData){
	    this.dataLength = realData.length;
	    this.fftDataLength = FourierTransform.nextPowerOfTwo(this.dataLength);
	    ...
	    for(int i=this.dataLength; i<this.fftDataLength; i++)this.complexData[i].reset(0.0D, 0.0D);
	    this.dataLengthOriginal = this.dataLength;
    ...
    public void setFftData(double[] fftdata){
        ...
        this.dataLength = fftdata.length/2;
	    this.dataLengthOriginal = this.dataLength;
	    this.fftDataLength = FourierTransform.nextPowerOfTwo(this.dataLength);

	    this.fftData = new double[2*this.fftDataLength];
	    for(int i=0; i<2*this.dataLength; i++)this.fftData[i] = fftdata[i];
	    for(int i=2*this.dataLength; i<2*this.fftDataLength; i++)this.fftData[i] = 0.0D;
	    this.fftDataSet = true;
	...
	public int getOriginalDataLength(){
		return this.dataLengthOriginal;
	}
	...
	private void checkSegmentDetails(){
	    ...
	    if(this.dataLength<2)throw new IllegalArgumentException("More than one point, MANY MORE, are needed");
        ...
        if(this.segNumSet && !this.overlap){
            if(this.dataLength % this.segmentNumber == 0){
                int segL = this.dataLength/this.segmentNumber;
                if(FourierTransform.checkPowerOfTwo(segL)){
                    ...
                    this.segmentLength = this.dataLength;
                    this.segLenSet = true;
                }
            }
            else{
        ...
        if(this.segLenSet && !this.overlap){
            if(this.dataLength % this.segmentLength == 0){
                 if(FourierTransform.checkPowerOfTwo(this.segmentLength)){
                    this.segmentNumber = this.dataLength/this.segmentLength;
                    this.segNumSet = true;
                }
                else{
                    System.out.println("segment length is not an integer power of two");
        ...
        if(this.segNumSet && this.overlap){
            if(this.dataLength % (this.segmentNumber+1) == 0){
                int segL = 2*this.dataLength/(this.segmentNumber+1);
                if(FourierTransform.checkPowerOfTwo(segL)){
                    this.segmentLength = segL;
                    this.segLenSet = true;
                }
        ...
        if(this.segLenSet && this.overlap){
            if((2*this.dataLength) % this.segmentLength == 0){
                 if(FourierTransform.checkPowerOfTwo(this.segmentLength)){
                    this.segmentNumber = (2*this.dataLength)/this.segmentLength - 1;
                    this.segNumSet = true;
                }
                else{
                    System.out.println("segment length is not an integer power of two");
	    ...
	    if(this.segmentNumber==1){
	        ...
	        if(!FourierTransform.checkPowerOfTwo(this.dataLength)){
	            boolean test0 = true;
	            boolean test1 = true;
	            boolean test2 = true;
	            int newL = 0;
	            ...
	            while(test0){
	                ...
	                if(FourierTransform.checkPowerOfTwo(newL) && (this.dataLength % ii)==0){
	                    test0 = false;
	                    segLno = newL;
	                    segNno = ii;
	                }
	            ...
	            // overlap option
	            while(test0){
	                newL = 2*(this.dataLength/(ii+1));
	                if(FourierTransform.checkPowerOfTwo(newL) && (this.dataLength % (ii+1))==0){
	                    test0 = false;
	                    segLov = newL;
	                    segNov = ii;
	                }
                ...
                if(setSegment && (this.dataLengthOriginal-segL <= this.fftDataLength - this.dataLengthOriginal)){
	                ...
	                this.dataLength = this.fftDataLength;
	                this.warning[8] = true;
	            }
	        }
	    }
	...
	private void printWarnings(FileOutput fout){
	    ...
	    if(warning[8]){
	        fout.println("WARNING!");
	        fout.println("Data length was not an integer power of two");
            fout.println("Data has been padded with " + (this.dataLength-this.dataLengthOriginal) + " zeros to give an integer power of two length");
            fout.println();
        }
	}
    ...
    public void transform(){
	    ...
	    if(this.dataLength!=this.fftDataLength){
	        System.out.println("Fast Fourier Transform data length ," + this.dataLength + ", is not an integer power of two");
	        System.out.println("WARNING!!! Data has been padded with zeros to fill to nearest integer power of two length");
	    }
	...
	public double[][] powerSpectrum(){
		...
		if(!overlap && this.segmentNumber<2){
            ...
            // perform fft
            double[] hold = new double[this.dataLength*2];
            for(int i=0; i<this.dataLength*2; i++)hold[i] = this.fftDataWindow[i];
            basicFft(hold, this.dataLength, isign);
            for(int i=0; i<this.dataLength*2; i++)this.transformedDataFft[i] = hold[i];
	...
	public double[][] powerSpectrum(String fileName){
        ...
        this.psdNumberOfPoints = this.segmentLength/2;
        this.powerSpectrumEstimate = new double[2][this.psdNumberOfPoints];
        this.dataLength =  FourierTransform.calcDataLength(this.overlap, this.segmentLength, this.segmentNumber);
        this.fftDataLength = this.dataLength;
    ...
    public void plotPowerLog(int lowPoint, int highPoint, String graphTitle){
        if(!this.powSpecDone){
            System.out.println("plotPowerLog - powerSpectrum has not been called - no plot displayed");
		}
		else{
	        int n = this.powerSpectrumEstimate[0].length-1;
		    if(lowPoint<0 || lowPoint>=n)lowPoint=0;
    ...
    public double[][] correlate(double[] data){
        ...
        if(nLen!=this.dataLengthOriginal)throw new IllegalArgumentException("The two data sets to be correlated are of different length");
    ...
    private double[][] correlation(int nLen){
        ...
        this.correlationArray = new double[2][nLen-1];
        j=0;
		for(int i=nLen/2+1; i<nLen; i++){
		    this.correlationArray[1][j] = hold3[2*i]/nLen;
		    j++;
        }
	    ...
	    // calculate time lags
		this.correlationArray[0][0]= -(double)(nLen/2-1)*this.deltaT;
	...
	public double[][] shortTime(int windowLength){
        ...
        if(!this.fftDataSet)throw new IllegalArgumentException("No data has been entered for the Fast Fourier Transform");
	    if(windowLength>this.dataLengthOriginal)throw new IllegalArgumentException("The window length, " + windowLength + ", is greater than the data length, " + this.dataLengthOriginal + ".");
        ...
        //  first column = blank cell followed by frequency vector
        //  each cell is then the mean square amplitude at that frequency and time
        this.numShortTimes = this.dataLengthOriginal - windowLength + 1;
    ...
    public void printShortTime(String filename){
        if(!this.shortTimeDone){
		    ...
		    fout.println(title);
		    fout.printtab("Data length = ");
		    fout.println(this.dataLengthOriginal);
